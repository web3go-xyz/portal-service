...
import { CDPLoanActionKlineItem, CDPLoanActionKlineResponse } from '../viewModel/cdp/CDPLoanActionKlineResponse';
...
export class CDPAnalysisService {
  ...
  async getCDPRule(): Promise<CDPRule> {
    if (CDPAnalysisService.CDP_RULE_INSTANCE == null) {
      ...
      let cdpRule = new CDPRule(chainState.requiredCollateralRatioPercentage, chainState.liquidationRatioPercentage, chainState.liquidationPenaltyPercentage);
  ...
  async getChainStatistic(chain: string, symbol: string): Promise<CDChainStatisticResponse> {
  ...
  async getLoanActionList(request: CDPLoanActionRequest): Promise<CDPLoanActionResponse> {

    let selectFields = ['id', 'chain', 'tokenId', 'accountId', 'actionId', 'type', 'subType', 'timestamp', , 'debitAdjustment', 'debit', 'collateralAdjustment', 'collateral', 'debitFormat', 'collateralFormat', 'collateralPrice', 'ratioPercentage', 'status', 'debitAdjustment_balance', 'collateralAdjustment_balance', 'blockNumber', 'blockId', 'extrinsicId'];
  ...
  async getLoanPositionStatistic(request: CDPLoanPositionRequest): Promise<CDPLoanPositionStatisticResponse> {
    ...
    let selectFields = ['actionId', 'type', 'timestamp', 'debitAdjustment', 'debit', 'collateralAdjustment', 'collateral', 'debitFormat', 'collateralFormat', 'collateralPrice'];
    ...
    if (loanActions && loanActions.length > 0) {
      ...
      loanActions.forEach(la => {
        let collateralChanged = CDPRule.formatCollateral(la.collateralAdjustment) * la.collateralPrice;
        let debitChanged = CDPRule.formatDebit(la.debitAdjustment);
        console.log('timestamp', la.timestamp, ",collateralChanged:", collateralChanged, ",debitChanged:", debitChanged);
      ...
      resp.cdpBalance = CDPRule.formatCollateral(lastLoanAction.collateral) * lastLoanAction.collateralPrice - CDPRule.formatDebit(lastLoanAction.debit);
    }
    resp.lifetimeProfit = resp.cdpBalance + resp.valueWithdraw - resp.valueInvested;

    MyLogger.log("getLoanPositionStatistic result:" + JSON.stringify(resp));

    return resp;
  }
  async getLoanActionKline(request: CDPLoanActionKlineRequest): Promise<CDPLoanActionKlineResponse> {
    ...
    let cacheKey: string = "kline_" + request.accountId + "_" + request.timeInterval + "_";
    cacheKey +=
      CDPPriceKeeper.formatTimeInterval(request.startTime, request.timeInterval).toString();
    cacheKey += "_";
    cacheKey += CDPPriceKeeper.formatTimeInterval(request.endTime, request.timeInterval).toString();
    ...
    let priceRange = await CDPPriceKeeper.getPriceRange(actual_startTime, actual_endTime);
    ...
    for (let dataIndex = 0; dataIndex < loanActions.length; dataIndex++) {
      ...
      let timeIndicator = CDPPriceKeeper.formatTimeInterval(timestamp, request.timeInterval);
    ...
    let currentTimeIndicator = CDPPriceKeeper.formatTimeInterval(actual_startTime, request.timeInterval);
    console.log("currentTimeIndicator:", currentTimeIndicator, ',actual_startTime:', actual_startTime);

    let timeInterval = CDPPriceKeeper.getTimeIntervalValue(request.timeInterval);
    ...
    while (currentTimeIndicator < actual_endTime) {
      ...
      if (formatedLoanActions[currentTimeIndicator]) {
        let findRecords = formatedLoanActions[currentTimeIndicator] as LoanAction[];
        for (let index = 0; index < findRecords.length; index++) {
          ...
          newItem.collateralAdjustmentFormat = CDPRule.formatCollateral(la.collateralAdjustment);
          newItem.collateralFormat = la.collateralFormat;
          newItem.debitAdjustmentFormat = CDPRule.formatDebit(la.debitAdjustment);
        ...
        if (lastRecord) {
          if (lastRecord.collateral > 0) {
            ...
            newItem.ratioPercentage = cdpRule.getLoanRatioPercentage(lastRecord.collateral, newItem.collateralPrice, lastRecord.debit)
